...
func formatJobDiff(job api.JobDiff, verbose bool, ui terminal.UI) {
...
func formatTaskGroupDiff(tg *api.TaskGroupDiff, tgPrefix int, verbose bool, ui terminal.UI) {
...
// task name output.
func formatTaskDiff(task *api.TaskDiff, startPrefix, taskPrefix int, verbose bool, ui terminal.UI) {
...
func formatFieldDiff(diff *api.FieldDiff, startPrefix, keyPrefix, valuePrefix int, ui terminal.UI) {
...
func formatObjectDiff(diff *api.ObjectDiff, startPrefix, keyPrefix int, ui terminal.UI) {
	...
	// Nil pointer check
	var fields []*api.FieldDiff
	var objects []*api.ObjectDiff
	if diff.Fields == nil {
...
func formatDryRun(resp *api.JobPlanResponse, job *api.Job, ui terminal.UI) {
	...
	if resp.FailedTGAllocs == nil {
		...
		for _, tg := range sorted {
			...
			ui.Warning(fmt.Sprintf("%sTaskGroup %q (failed to place %d %s):\n",
				strings.Repeat(" ", 2), tg, metrics.CoalescedFailures, noun))
			formatAllocMetrics(metrics, strings.Repeat(" ", 4), ui)
		}
	}

	if rolling != nil {
		ui.Success(fmt.Sprintf("\n- Rolling update, next evaluation will be in %s.", rolling.Wait.String()))
	}
...
func formatAllocMetrics(metrics *api.AllocationMetric, prefix string, ui terminal.UI) {
...
func formatPreemptions(ui terminal.UI, resp *api.JobPlanResponse) {
