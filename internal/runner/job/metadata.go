...
func (r *Runner) setHCLMeta(job string) string {
	// Parse the job HCL to find any existing meta block or attribute.
	file, diags := hclsyntax.ParseConfig([]byte(job), "", hcl.Pos{Line: 1, Column: 1})
	if diags.HasErrors() {
		return job
	}
	// Find the job block. If none exists, the original job is returned.
	...
	for _, b := range content.Blocks {
		// Find the meta block or attribute within the job block, if it exists
		...
		// If no meta block or attribute exists, no merge is necessary
	...
	// Parse the HCL again with hclwrite so it can be modified
	...
	// If we extracted the meta data from a block, delete it so we don't result in invalid HCL
	if deleteBlock {
