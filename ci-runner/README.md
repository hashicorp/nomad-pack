...
```
nomad-pack run ci-runner --var message="Hola Mundo!"
```
...
balance the Consul service `ci-runner-service`, which is the default value
...
- `consul_service_tags` (list of string) - The consul service name for the
  ci-runner application
- `consul_service_name` (string) - The consul service name for the ci-runner
  application
